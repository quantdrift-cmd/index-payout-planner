...
export const MultiplierReference = ({ activeTab }: MultiplierReferenceProps) => {
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4">
      <h3 className="text-sm font-medium text-foreground mb-3">Contract Multipliers & Strike Intervals</h3>
      ...
      <p className="text-xs text-muted-foreground mt-3">
        {activeTab === 'stocks' 
          ? "All stock options have a standard multiplier of 100. Strike interval shows the price gap between available strikes."
          : "Multiplier determines dollar value per point. Strike interval shows the price gap between available strikes."}
      </p>
    </div>
  );
};
